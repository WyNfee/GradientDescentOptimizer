...
%adadelta updater
t_adadelta_previous_weight_sum = zeros(size(t_packedweightforSGD));
...
if(g_do_gradient_descent == true)
    ...
    for i = 1: t_iteration_time
        ...
        %the update delta of gradient
        t_current_update_weight_sum = t_adadelta_previous_weight_sum + t_gradient_param;
        
        %compute the rms of previous update and current update
        t_current_update_weight_rms = sqrt(sum(sum(t_current_update_weight_sum.^2)));
        t_previous_update_weight_rms = sqrt(sum(sum(t_adadelta_previous_weight_sum.^2)));
        
        %compute the updater
        t_adadelta_updater = (t_previous_update_weight_rms/t_current_update_weight_rms) * t_gradient_param;
        
        %store the sum for next iteration
        t_adadelta_previous_weight_sum = t_current_update_weight_sum;
        
        %simply compute the gradient
        t_packedweightforSGD = t_packedweightforSGD - t_adadelta_updater;
    ...
    if(s == 'y')
        save('data_adadelta.mat', 't_packedweightforSGD', 't_record_cost_data');
        fprintf('Data Saved\n');
    else
    ...
    load('data_adadelta.mat');
    
    %prepare the data for plot
    t_cost_data_size = length(t_record_cost_data);
    t_cost_data_adadelta = zeros(t_cost_data_size, 2);
    
    for i = 1 : t_cost_data_size
        
        t_cost_data_adadelta(i, 1) = i;
        t_cost_data_adadelta(i, 2) = t_record_cost_data(i);
        
    end
    
    %plot the data
    plot(t_cost_data_adadelta(:,1), t_cost_data_adadelta(:,2),'--');
    
    %Save the data to compare with other learning algorithm
    s = input('save the plot data?, y to save:','s');
    if(s == 'y')
        save('data_lost_adadelta.mat', 't_cost_data_adadelta');
        fprintf('Data Saved\n');
    else
         fprintf('No Data Saved\n');
    end
