...
%using common gradient descent approach(No optimizer)
...
if(g_do_gradient_descent == true)
    ...
    for i = 1: t_iteration_time
        ...
        t_packedweightforSGD = t_packedweightforSGD - g_h_learning_rate * t_gradient_param;
    ...
    if(s == 'y')
        save('data_no_optimizer.mat', 't_packedweightforSGD', 't_record_cost_data');
        fprintf('Data Saved\n');
    else
    ...
    t_cost_data_no_optimizer = zeros(t_cost_data_size, 2);
    
    for i = 1 : t_cost_data_size
        
        t_cost_data_no_optimizer(i, 1) = i;
        t_cost_data_no_optimizer(i, 2) = t_record_cost_data(i);
        
    end
    
    %plot the data
    plot(t_cost_data_no_optimizer(:,1), t_cost_data_no_optimizer(:,2),'--');
    
    %Save the data to compare with other learning algorithm
    s = input('save the plot data?, y to save:','s');
    if(s == 'y')
        save('data_lost_no_optimizer.mat', 't_cost_data_no_optimizer');
        fprintf('Data Saved\n');
    else
         fprintf('No Data Saved\n');
    end
